...
static int test_main01(struct lua_State *L)
{
    ...
    printf("test_main01 luaL_tointeger value = %lld\n", i);
    return 0;
}

void p1_test_result01()
{ // nwant = 0; and nresult = 0;
...
static int test_main02(struct lua_State *L)
{
    ...
    printf("test_main02 luaL_tointeger value = %lld\n", i);
    return 1;
}

void p1_test_result02()
{ // nwant = 0; and nresult = 1;
    struct lua_State *L = luaL_newstate();
...
void p1_test_result04()
{ // nwant = 1; and nresult = 0;
    struct lua_State *L = luaL_newstate();
...
void p1_test_result05()
{ // nwant = 1; and nresult = 1;
    struct lua_State *L = luaL_newstate();
...
{ // nwant = 1; and nresult > 1;
    ...
    printf("p1_test_result06 top value:%lld\n", v);
    luaL_pop(L);
...
{ // nwant > 1; and nresult > 0;
    ...
    for (i = 0; i < test_result08_nwant; i++)
    {
        if (i != 0)
        {
            ...
            luaL_pop(L);
            printf("p1_test_result08 stack_idx:%d integer:%lld\n", test_result08_nwant - i, integer);
        }
        else
        {
            int isnil = luaL_isnil(L, -1);
...
{ // call count < LUA_MAXCALLS
    ...
    printf("p1_test_nestcall01 result = %lld stack_size:%d\n", luaL_tointeger(L, -1), luaL_stacksize(L));
    luaL_pop(L);

    luaL_close(L);
}

void p1_test_nestcall02()
{ // call count >= LUA_MAXCALLS
}
